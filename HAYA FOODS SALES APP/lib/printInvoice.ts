...
export function printInvoice(invoice: PrintableInvoice): boolean {
  ...
  <style>
    * { box-sizing: border-box; } body { margin: 0; font-family: Arial, Helvetica, sans-serif; color: #17211e; background: #f4f7f6; }
    ...
    <section class="top"><div><h1 class="brand">HAYA FOODS PRODUCTS (PVT) LTD</h1><p class="sub">No.239/7, Muhamathiya Nagar, Railway Line, Madcove, Trincomalee, Sri Lanka</p><p class="sub">+94 78 811 0101 &nbsp;|&nbsp; info@hayafoods.lk</p></div><div><h2 class="invoice-title">SALES INVOICE</h2><div class="number">${escapeHtml(invoice.invoice_number)}</div></div></section>
